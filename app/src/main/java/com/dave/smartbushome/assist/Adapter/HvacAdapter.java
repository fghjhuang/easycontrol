 ...
 */
public class HvacAdapter extends BaseAdapter{
    private Context context;
    private List<Savehvac> buttonarray=new ArrayList<Savehvac>();
    private int size;
    private LayoutInflater inflater=null;
    private HashMap<Integer, View> mView ;
    private String[] colorarray={"FF2176BC", "FF8E4E87","FFB62F32","FF7BC176","FFEB6A68","FFF08519","FFFAC65A"};
    public HvacAdapter(final Context context, List<Savehvac> list){
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(view==null)
        {
            ...
            txt.setText(buttonarray.get(position).hvac_remark);
            ceb.setImageDrawable(context.getResources().getDrawable(getResourdIdByResourdName(context,"hvacitem_icon")));
            view.setBackgroundColor(ToColor(colorarray[position%7]));
            mView.put(position, view);
        }
        return view;
    }
    public Savehvac getselected(int position){
        return buttonarray.get(position);
    }
