...
import com.dave.smartbushome.database.Savemarcobutton;
import com.dave.smartbushome.database.Savemedia;
...
public class MediaAdapter extends BaseAdapter{
    ...
    private List<Savemedia> buttonarray=new ArrayList<Savemedia>();
    private int size;
    private LayoutInflater inflater=null;
    private HashMap<Integer, View> mView ;
    private String[] colorarray={"FF2176BC", "FF8E4E87","FFB62F32","FF7BC176","FFEB6A68","FFF08519","FFFAC65A"};
    public MediaAdapter(final Context context, List<Savemedia> list){
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(view==null)
        {
            ...
            txt.setText(buttonarray.get(position).media_statement);
            ceb.setImageDrawable(context.getResources().getDrawable(getResourdIdByResourdName(context, buttonarray.get(position).media_icon)));
            view.setBackgroundColor(ToColor(colorarray[position%7]));
            mView.put(position, view);
        }
        return view;
    }
    public Savemedia getselected(int position){
        return buttonarray.get(position);
    }
