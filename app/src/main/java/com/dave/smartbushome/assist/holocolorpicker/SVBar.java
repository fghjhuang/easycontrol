...
package com.dave.smartbushome.assist.holocolorpicker;
...
public class SVBar extends View {
	 ...
	 * @param color
	 */
	public void setColor(int color,boolean ifreset) {
		int x1, y1;
		if(mOrientation) {
			x1 = (mBarLength + mBarPointerHaloRadius);
			y1 = mBarThickness;
		...
		shader = new LinearGradient(mBarPointerHaloRadius, 0,
				...
				Shader.TileMode.CLAMP);
		mBarPaint.setShader(shader);
		/**add this to set the middle value**/
		if(ifreset){
			mBarPointerPosition = (mBarLength / 2) + mBarPointerHaloRadius;
			calculateColor(mBarPointerPosition);
		}else{
	...
	protected void onRestoreInstanceState(Parcelable state) {
		...
		setColor(Color.HSVToColor(savedState.getFloatArray(STATE_COLOR)),false);
