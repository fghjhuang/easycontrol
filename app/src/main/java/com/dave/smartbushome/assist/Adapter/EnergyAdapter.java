...
import com.dave.smartbushome.database.Saveenergy;
...
public class EnergyAdapter extends BaseAdapter{
    ...
    private List<Saveenergy> buttonarray=new ArrayList<Saveenergy>();
    private int size;
    private LayoutInflater inflater=null;
    private HashMap<Integer, View> mView ;
    private String[] colorarray={"FF2176BC", "FF8E4E87","FFB62F32","FF7BC176","FFEB6A68","FFF08519","FFFAC65A"};
    public EnergyAdapter(final Context context, List<Saveenergy> list){
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(view==null)
        {
            ...
            txt.setText(buttonarray.get(position).energyname);
            ceb.setImageDrawable(context.getResources().getDrawable(getResourdIdByResourdName(context,"energy_icon")));
            view.setBackgroundColor(ToColor(colorarray[position%7]));
            mView.put(position, view);
        }
        return view;
    }
    public Saveenergy getselected(int position){
        return buttonarray.get(position);
    }
